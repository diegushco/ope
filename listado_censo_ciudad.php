...
$consulta="select cc.Id_Censo as idCenso, cvi.Id_Vivienda as idvi,
...
";
		...
		?>
										...
										<tr>
											...
											<td class="hidden-phone"><?php echo $row["aldea"]; ?></td>
...
include("guardaBitacora.php");
		...
		if($row=mysql_fetch_array($sql)){
			...
			while($row=mysql_fetch_array($sql)){
				...
				$consulta1="select count(raa.Id_Actividad) as cantidad 
				from ra_actividad raa, ra_organismo_actividad roa,
				ra_otros_organismo roo 
				where roa.Id_OtrosOrganismo=roo.Id_OtrosOrganismo and
				raa.Id_Actividad=roa.Id_Actividad and
				roo.Id_OtrosOrganismo like '%".$row['Id_OtrosOrganismo']."%' ";
