	...
	<div class="row-fluid">
		...
		<form class="form-inline">
		   <div class="control-group">
			...
			<a href="javascript: menu('listado_censo_tenencia')" class="btn btn-small btn-info">
...
$consulta="select cten.Nombre as tenencia, cc.Id_Censo as idCenso, cvi.Id_Vivienda as idvi,
...
";
		...
		?>
										...
										<tr>
											...
											<td class="hidden-phone"><?php echo $row["tenencia"]; ?></td>
...
include("guardaBitacora.php");
					bitacora("Busca censo reporte por tenencia","Buscar",$conexion);
		...
		$cons="
					...
					select DISTINCT cten.Nombre as Nombre, cten.Id_Tenencia as Id_Tenencia
					from cs_afectado csa, cs_otra_propiedad cop, cs_refugio_afectado cra, cs_refugio cr, cs_censo cc, cs_evento ce, funcionario fu, aldea al, cs_motivo cm, cs_condicion_familiar cf, cs_vivienda cvi, cs_danho cda, cs_tenencia cten, cs_tipo ctip, cs_parte cpar, cs_material_elaboracion cmel1, cs_material_elaboracion cmel2, cs_material_elaboracion cmel3
					, ciudad ciu, parroquia parr, municipio mun
					where
					mun.Id_Municipio=parr.Id_Municipio and
		...
		if($row=mysql_fetch_array($sql)){
			...
			while($row=mysql_fetch_array($sql)){
				...
				$consulta1="
				select count(cten.Id_Tenencia) as cantidad, cten.Nombre
					from cs_afectado csa, cs_otra_propiedad cop, cs_refugio_afectado cra, cs_refugio cr, cs_censo cc, cs_evento ce, funcionario fu, aldea al, cs_motivo cm, cs_condicion_familiar cf, cs_vivienda cvi, cs_danho cda, cs_tenencia cten, cs_tipo ctip, cs_parte cpar, cs_material_elaboracion cmel1, cs_material_elaboracion cmel2, cs_material_elaboracion cmel3
					, ciudad ciu, parroquia parr, municipio mun
					where
					mun.Id_Municipio=parr.Id_Municipio and
					...
					cten.Id_Tenencia like '%".$row['Id_Tenencia']."%' ";
					//echo $consulta1;
...
$_SESSION["nombres"]=$nombres;
 ?>
							...
							<img id="grafico" src="graficoBarTenencia.php?var=<?php echo time(); ?>" />
							</div>
	</div>	
...
function traeListado(){
			...
			objeto_ajax.open("GET", "trae_CensoTenencia.php?hora="+hora+"&fecha1="+fecha1+"&fecha2="+fecha2+"&idbus="+aldea1,true);
