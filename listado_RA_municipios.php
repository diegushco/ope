	...
	<div class="row-fluid">
		...
		<form class="form-inline">
		   <div class="control-group">
			...
			<a href="javascript: menu('listado_RA_municipios')" class="btn btn-small btn-info">
...
bitacora("Busca reporte RA ".$row["folio"],"Buscar",$conexion);
		...
		$aco1=0;
		include("conexion.php");	
		$conm="select DISTINCT muni.Nombre as Nombre, muni.Id_Municipio as idmu
				from ra_actividad raa, 
				aldea ald,					
				ciudad ciud,
				parroquia parr, 
		...
		if($row=mysql_fetch_array($sql)){
			...
			while($row=mysql_fetch_array($sql)){
				...
				$consulta1="select count(muni.Nombre) as cantidad
				from ra_actividad raa, 
				aldea ald,					
				ciudad ciud,
				parroquia parr, 
				...
				ciud.Id_Parroquia=parr.Id_Parroquia and
				parr.Id_Municipio=muni.Id_Municipio and
				muni.Id_Municipio like '%".$row['idmu']."%' ";
...
$_SESSION["nombres"]=$nombres;
 ?>
							...
							<img id="grafico" src="graficoBarM.php?var=<?php echo time(); ?>" />
							</div>
	</div>	
...
function traeListado(){
			...
			objeto_ajax.open("GET", "trae_RAOMuni.php?hora="+hora+"&fecha1="+fecha1+"&fecha2="+fecha2+"&muni="+aldea1,true);
