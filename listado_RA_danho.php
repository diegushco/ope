		...
		<form class="form-inline">
		   <div class="control-group">
			...
			<select data-placeholder="Seleccione" id="aldea" name="aldea" onchange="aldeasel1(this)"  class="chzn-select" >
											...
											$sql=mysql_query("SELECT * FROM ra_danho order by Nombre asc",$conexion);  
											if($row=mysql_fetch_array($sql)){
												$sql=mysql_query("SELECT * FROM ra_danho order by Nombre asc",$conexion);  
												while($row=mysql_fetch_array($sql)){
													echo '<option value="'.$row['Id_Danho'].'" >'.$row['Nombre'].'</option>';
												}
											}
											mysql_close();		
											?>
			...
			<a href="javascript: menu('listado_RA_danho')" class="btn btn-small btn-info">
...
<?php
			include("conexion.php");
			$consulta="select rdn.Nombre as aldea, raa.Id_Actividad as actividad, raa.Fecha as fecha, raa.Numero_Folio as folio, rat.Nombre as tipo, rco.Nombre as condicion
...
bitacora("Busca reporte RA por condicion ","Buscar",$conexion);


		$aco1=0;
		include("conexion.php");	
		$conm="select DISTINCT rdn.Id_Danho as idti, rct.Nombre as Nombre
				from ra_actividad raa, 
				ra_tipo rat,
				aldea ald,
				ra_accion_actividad rac,
		...
		if($row=mysql_fetch_array($sql)){
			...
			while($row=mysql_fetch_array($sql)){
				...
				$consulta1="select count(rct.Id_AccionTomada) as cantidad
				from ra_actividad raa, 
				ra_tipo rat,
				aldea ald,
				ra_accion_actividad rac,
...
$_SESSION["nombres"]=$nombres;
 ?>
							...
							<img id="grafico" src="graficoBarDA.php?var=<?php echo time(); ?>" />
							</div>
	</div>	
...
function traeListado(){
			...
			objeto_ajax.open("GET", "trae_RADnaho.php?hora="+hora+"&fecha1="+fecha1+"&fecha2="+fecha2+"&muni="+aldea1,true);
