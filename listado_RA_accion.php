<style type="text/css">
	...
	<div class="page-header position-relative titulo">
		<h2>
			Reporte de RA por Accion Tomada
		</h2>
	</div><!--/.page-header-->
	...
	<div class="row-fluid">
		...
		<form class="form-inline">
		   <div class="control-group">
			...
			<select data-placeholder="Seleccione" id="aldea" name="aldea" onchange="aldeasel1(this)"  class="chzn-select" >
											...
											if($row=mysql_fetch_array($sql)){
												...
												while($row=mysql_fetch_array($sql)){
													echo '<option value="'.$row['Id_AccionTomada'].'" >'.$row['Nombre'].'</option>';
												}
											}
											mysql_close();		
											?>
			...
			<a href="javascript: menu('listado_RA_accion')" class="btn btn-small btn-info">
...
bitacora("Busca reporte RA por condicion ","Buscar",$conexion);
		...
		$aco1=0;
		include("conexion.php");	
		$conm="select DISTINCT rct.Id_AccionTomada as idti, rct.Nombre as Nombre
				from ra_actividad raa, 
				ra_tipo rat,
				aldea ald,
				ra_accion_actividad rac,
		...
		if($row=mysql_fetch_array($sql)){
			...
			while($row=mysql_fetch_array($sql)){
				...
				ciud.Id_Parroquia=parr.Id_Parroquia and
				parr.Id_Municipio=muni.Id_Municipio and
				rct.Id_AccionTomada like '%".$row['idti']."%' ";
...
$_SESSION["nombres"]=$nombres;
 ?>
							...
							<img id="grafico" src="graficoBarCo.php?var=<?php echo time(); ?>" />
							</div>
	</div>	
...
function traeListado(){
			...
			objeto_ajax.open("GET", "trae_RAAccion.php?hora="+hora+"&fecha1="+fecha1+"&fecha2="+fecha2+"&muni="+aldea1,true);
