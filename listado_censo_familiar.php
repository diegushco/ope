<style type="text/css">
	...
	<div class="page-header position-relative titulo">
		<h2>
			Reporte de Censo por Condicion Familiar
		</h2>
	</div><!--/.page-header-->
		...
		<form class="form-inline">
		   <div class="control-group">
			...
			<label>
				...
				<span class="lbl">Cond. Familiar</span>
			</label>
			<select data-placeholder="Seleccione" id="aldea" name="aldea" onchange="aldeasel1(this)"  class="chzn-select" >
											...
											$sql=mysql_query("SELECT * FROM cs_condicion_familiar order by Nombre asc",$conexion);  
											if($row=mysql_fetch_array($sql)){
												$sql=mysql_query("SELECT * FROM cs_condicion_familiar order by Nombre asc",$conexion);  
												while($row=mysql_fetch_array($sql)){
													echo '<option value="'.$row['Id_CondicionFamiliar'].'" >'.$row['Nombre'].'</option>';
												}
											}
											mysql_close();		
											?>
			...
			<a href="javascript: menu('listado_censo_familiar')" class="btn btn-small btn-info">
...
$consulta="select cf.Nombre as condicion, cc.Id_Censo as idCenso, cvi.Id_Vivienda as idvi,
...
, ciudad ciu, parroquia parr, municipio mun, cs_condicion csc
where
csc.Id_Condicion=cvi.Id_Condicion and
...
";
		...
		?>
										...
										<tr>
											...
											<td class="hidden-phone"><?php echo $row["condicion"]; ?></td>
...
include("guardaBitacora.php");
					bitacora("Busca censo reporte por condicion familiar","Buscar",$conexion);
		...
		$cons="
					...
					select DISTINCT cf.Nombre as Nombre, cf.Id_CondicionFamiliar as Id_CondicionFamiliar
					from cs_afectado csa, cs_otra_propiedad cop, cs_refugio_afectado cra, cs_refugio cr, cs_censo cc, cs_evento ce, funcionario fu, aldea al, cs_motivo cm, cs_condicion_familiar cf, cs_vivienda cvi, cs_danho cda, cs_tenencia cten, cs_tipo ctip, cs_parte cpar, cs_material_elaboracion cmel1, cs_material_elaboracion cmel2, cs_material_elaboracion cmel3
					, ciudad ciu, parroquia parr, municipio mun, cs_condicion csc
					where
					csc.Id_Condicion=cvi.Id_Condicion and
		...
		if($row=mysql_fetch_array($sql)){
			...
			while($row=mysql_fetch_array($sql)){
				...
				$consulta1="
				select count(cf.Id_CondicionFamiliar) as cantidad, cf.Nombre
					from cs_afectado csa, cs_otra_propiedad cop, cs_refugio_afectado cra, cs_refugio cr, cs_censo cc, cs_evento ce, funcionario fu, aldea al, cs_motivo cm, cs_condicion_familiar cf, cs_vivienda cvi, cs_danho cda, cs_tenencia cten, cs_tipo ctip, cs_parte cpar, cs_material_elaboracion cmel1, cs_material_elaboracion cmel2, cs_material_elaboracion cmel3
					, ciudad ciu, parroquia parr, municipio mun, cs_condicion csc
					where
					csc.Id_Condicion=cvi.Id_Condicion and
					...
					cf.Id_CondicionFamiliar like '%".$row['Id_CondicionFamiliar']."%' ";
					//echo $consulta1;
...
$_SESSION["nombres"]=$nombres;
 ?>
							...
							<img id="grafico" src="graficoBarConFamiliar.php?var=<?php echo time(); ?>" />
							</div>
	</div>	
...
function traeListado(){
			...
			objeto_ajax.open("GET", "trae_CensoFamiliar.php?hora="+hora+"&fecha1="+fecha1+"&fecha2="+fecha2+"&idbus="+aldea1,true);
