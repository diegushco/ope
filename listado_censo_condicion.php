<style type="text/css">
	...
	<div class="page-header position-relative titulo">
		<h2>
			Reporte de Censo por Condicion de Vivienda
		</h2>
	</div><!--/.page-header-->
	...
	<div class="row-fluid">
		...
		<form class="form-inline">
		   <div class="control-group">
			...
			<a href="javascript: menu('listado_censo_condicion')" class="btn btn-small btn-info">
...
$consulta="select csc.Nombre as condicion, cc.Id_Censo as idCenso, cvi.Id_Vivienda as idvi,
...
include("guardaBitacora.php");
					bitacora("Busca censo reporte por condicion","Buscar",$conexion);
		...
		$cons="
					...
					select DISTINCT csc.Nombre as Nombre, csc.Id_Condicion as Id_Condicion
					from cs_afectado csa, cs_otra_propiedad cop, cs_refugio_afectado cra, cs_refugio cr, cs_censo cc, cs_evento ce, funcionario fu, aldea al, cs_motivo cm, cs_condicion_familiar cf, cs_vivienda cvi, cs_danho cda, cs_tenencia cten, cs_tipo ctip, cs_parte cpar, cs_material_elaboracion cmel1, cs_material_elaboracion cmel2, cs_material_elaboracion cmel3
					, ciudad ciu, parroquia parr, municipio mun, cs_condicion csc
					where
					csc.Id_Condicion=cvi.Id_Condicion and
		...
		if($row=mysql_fetch_array($sql)){
			...
			while($row=mysql_fetch_array($sql)){
				...
				$consulta1="
				select count(csc.Id_Condicion) as cantidad, csc.Nombre
					from cs_afectado csa, cs_otra_propiedad cop, cs_refugio_afectado cra, cs_refugio cr, cs_censo cc, cs_evento ce, funcionario fu, aldea al, cs_motivo cm, cs_condicion_familiar cf, cs_vivienda cvi, cs_danho cda, cs_tenencia cten, cs_tipo ctip, cs_parte cpar, cs_material_elaboracion cmel1, cs_material_elaboracion cmel2, cs_material_elaboracion cmel3
					, ciudad ciu, parroquia parr, municipio mun, cs_condicion csc
					where
					csc.Id_Condicion=cvi.Id_Condicion and
					...
					csc.Id_Condicion like '%".$row['Id_Condicion']."%' ";
					//echo $consulta1;
...
$_SESSION["nombres"]=$nombres;
 ?>
							...
							<img id="grafico" src="graficoBarCondicion.php?var=<?php echo time(); ?>" />
							</div>
	</div>	
...
function traeListado(){
			...
			objeto_ajax.open("GET", "trae_CensoCondicion.php?hora="+hora+"&fecha1="+fecha1+"&fecha2="+fecha2+"&idbus="+aldea1,true);
